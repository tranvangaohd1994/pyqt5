...
def wxfinished():
    ...
    if Config.metric:
        ...
        if do_am is not None and nhiet_do is not None:
            nhiet_do_str = nhiet_do.__str__()
            do_am_str = do_am.__str__()
            indoorTemp.setText(nhiet_do_str + u'°C')
            indoorHumi.setText(Config.LInsideHumi + do_am_str + u'%')
        else:
            # Loi :(
...
squares1.setGeometry(0, height - yscale * 325 , xscale * 340, yscale * 600)
...
#radar1rect = QtCore.QRect(3 * xscale, 344 * yscale + 20, 300 * xscale, 275 * yscale)
...
indoorTemp = QtGui.QLabel(frame1)
...
ypos += 75
indoorHumi = QtGui.QLabel(frame1)
